...
export default function Properti({ properti, filters }) {
    ...
    const items = properti.data || properti;

    const doSearch = useCallback((val) => {
        router.visit(route('properti.index'), {
            data: { search: val },
    ...
    const submit = (e) => {
        ...
        if (editingId) {
            form.put(route('properti.update', editingId), { preserveScroll: true, onSuccess });
        } else {
            form.post(route('properti.store'), { preserveScroll: true, onSuccess });
        }
    };

    const editItem = (item) => {
        setEditingId(item.id); setDetailId(null); setShowForm(true);
    ...
    return (
        <AppLayout title="Database Properti" description="Tanah, Bangunan, dan Rumah Negara BMN"
            actions={<button type="button" onClick={() => { setShowForm(v => !v); if (showForm) cancelEdit(); }} className="rounded-2xl bg-primary px-4 py-3 text-sm font-medium text-white">{showForm ? 'Tutup Form' : '+ Tambah'}</button>}
        >
            <Head title="Properti" />
            <div className="mb-6 space-y-6">
                ...
                <form onSubmit={submit} className="rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-700 p-5">
                    <div className="mb-5 flex items-center justify-between">
                        ...
                        <button type="button" onClick={cancelEdit} className="rounded-2xl border border-gray-300 dark:border-gray-600 px-3 py-2 text-sm text-gray-700 dark:text-gray-200">Batal</button>
                    </div>
                ...
                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
                    <div className="rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-700 p-5"><p className="text-sm text-gray-500">Total</p><p className="mt-2 text-3xl font-semibold text-gray-900 dark:text-gray-100">{properti.total || items.length}</p></div>
                    <div className="rounded-xl border border-gray-200 dark:border-gray-700 bg-amber-50 p-5"><p className="text-sm text-gray-500">🌍 Tanah</p><p className="mt-2 text-3xl font-semibold text-amber-600">{items.filter(i=>i.jenis_properti==='Tanah').length}</p></div>
                    <div className="rounded-xl border border-gray-200 dark:border-gray-700 bg-blue-50 p-5"><p className="text-sm text-gray-500">🏢 Bangunan</p><p className="mt-2 text-3xl font-semibold text-blue-600">{items.filter(i=>i.jenis_properti==='Bangunan').length}</p></div>
                    <div className="rounded-xl border border-gray-200 dark:border-gray-700 bg-purple-50 p-5"><p className="text-sm text-gray-500">🏠 Rumah Negara</p><p className="mt-2 text-3xl font-semibold text-purple-600">{items.filter(i=>i.jenis_properti==='Rumah Negara').length}</p></div>
                    <div className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-5"><input className={ic} placeholder="Cari properti..." value={keyword} onChange={handleSearch} /></div>
                </div>
                        ...
                        <article key={item.id} className="group cursor-pointer rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 overflow-hidden hover:shadow-lg transition-shadow" onClick={() => setDetailId(item.id)}>
                            <div className="h-32 bg-gray-100 dark:bg-gray-700 relative overflow-hidden">
                                {item.foto_url ? (
                                    ...
                                    <div className="flex items-center justify-center h-full"><span className="text-4xl">{jenisIcon(item.jenis_properti)}</span></div>
                                )}
                ...
                <Pagination data={properti} routeName="properti.index" />
            </div>
        </AppLayout>
    );
}

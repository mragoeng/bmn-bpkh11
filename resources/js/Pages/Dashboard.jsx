        ...
        <AppLayout title="Dashboard" description="Ringkasan cepat data master, transaksi terbaru, dan titik masuk utama untuk modul pencatatan BBM.">
            ...
            <div className="space-y-8">
                <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                    {stats.map((item) => (
                        <div key={item.label} className="rounded-xl border border-gray-200 bg-primary-pale/30 p-5">
                            <p className="text-sm text-primary">{item.label}</p>
                            <p className="mt-2 text-3xl font-bold text-primary-dark">{item.value}</p>
                ...
                <div className="grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
                    <div className="overflow-hidden rounded-xl border border-gray-200 bg-white">
                        <div className="border-b border-gray-200 bg-primary px-5 py-4">
                                ...
                                {latestTransactions.length ? (
                                    latestTransactions.map((item) => (
                                        <tr key={`${item.tanggal}-${item.nomor_polisi}`}>
                                            ...
                                            <td className="px-5 py-3 font-medium text-gray-900">{item.pegawai}</td>
                                            <td className="px-5 py-3 text-gray-600">{item.kendaraan}</td>
                                            <td className="px-5 py-3 text-gray-600">{item.liter}</td>
                                            <td className="px-5 py-3 font-medium text-gray-900">{item.total}</td>
                                        </tr>
                                    ))
                                ) : (
                                    <tr><td colSpan="5" className="px-5 py-8 text-center text-gray-400">Belum ada transaksi BBM terbaru.</td></tr>
                                )}
                            </tbody>
                        </table>
                    </div>
